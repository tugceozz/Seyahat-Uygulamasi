...
public class adana extends AppCompatActivity {
    BottomNavigationView adana;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_adana);
        adana=findViewById(R.id.adana2);
        adana.setOnItemSelectedListener(new NavigationBarView.OnItemSelectedListener() {
            @Override
            public boolean onNavigationItemSelected(@NonNull MenuItem item1) {
                switch (item1.getItemId()){
                    case R.id.muze:
                        getSupportFragmentManager().beginTransaction().replace(R.id.adana,new AdanaMuze()).commit();
                        break;
                    case R.id.restorant:
                        getSupportFragmentManager().beginTransaction().replace(R.id.adana,new AdanaRestoran()).commit();
                        break;
                    case R.id.doga:
                        getSupportFragmentManager().beginTransaction().replace(R.id.adana,new AdanaDoga()).commit();
                        break;
                }
                return true;
            }
